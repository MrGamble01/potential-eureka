/*
 * HV-183 — Tell the Fire Story said around the fire,
 * then Fire Went Out still hosted the telling.
 *
 * The story tooltip says "Tell it around the fire once a session."
 * finishAction logs "told around the fire." Fire Went Out dims the
 * barrel for 30s (G.fireOutUntil) and logs that it is cold and dark.
 * Clicking the telling while the barrel was dark still started the
 * 2s job, stamped hvStoryTold, and paid the dinner as if the fire
 * were still there to sit around.
 *
 * Distinct from HV-69 (Firewood relights the barrel), #842
 * (Fire Went Out vs the sky), #802 (Tell the Fire Story with
 * no story yet), HV-47 / hvstory (the dish and the once-a-session
 * stamp), #851 (Hold a Camp Meeting vs a dead barrel), and #870
 * (Hot Meal vs a dead barrel). This ticket is the telling vs a
 * dead fire. ui.js is not this ticket.
 *
...
ok(/Tell it around the fire/.test(cfg),
  'Tell the Fire Story still promises to tell it around the fire');
...
ok(storyAt >= 0 && fireAt >= 0 && storyAt < timeoutAt && fireAt < timeoutAt,
  'HV-183: doAction refuses a telling around a dead fire before the timer');
...
(async () => {
  ...
  ok(boot.heart && boot.dish === 9,
    `three sits still learn the story and dish 9 (${boot.dish})`);
  ...
  ok(!dark.job && !dark.told && dark.food === 10 && dark.tellings === 0,
    `HV-183: a dead fire does not host the telling (job ${dark.job}, told ${dark.told}, food ${dark.food}, tellings ${dark.tellings})`);
  ok(/fire is out|barrel|dark|lit/i.test(dark.added),
  ...
  ok(lit.food === 19 && lit.told && lit.tellings === 1,
    `three sits still pay dinner of 9 (food ${lit.food}, tellings ${lit.tellings})`);
