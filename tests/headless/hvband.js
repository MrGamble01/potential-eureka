/*
 * HV-246 — Theft said they raided your stash, then left the weather band.
 *
...
ok(/structures\.radio\s*=\s*false/.test(theftBlock),
  'HV-246: the theft effect takes the weather band');
...
(async () => {
  ...
  ok(!raid.radio && !raid.sky,
    `HV-246: a radio-only camp loses tomorrow's sky (radio=${raid.radio}, sky=${raid.sky})`);
