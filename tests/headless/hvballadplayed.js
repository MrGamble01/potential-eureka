/*
 * HV-281 — Play the Bridge Ballad said once a session, then a
 * second click still ran the job.
 *
 * The tooltip says "Play it once a session, and the hat by the
 * fire always fills before the last verse." finishAction already
 * logs "The ballad got its playing tonight — the tune keeps." and
 * no-ops. Clicking 🎸 after balladPlayed still started the 2s job
 * and charged the 30s lock as if the hat had filled twice.
 *
 * Distinct from HV-192 / #832 (a dead barrel still filled the hat),
 * HV-221 / #900 (no ballad set yet — three tellings first), HV-249
 * / #950 (the fire story already got its telling — the predecessor
 * link, not this one), HV-212 (the coffee can still bare). This
 * ticket is the ballad after it already got its playing. ui.js is
 * not this ticket.
 *
 *  A. Source: doAction names the balladPlayed gate before
 *     setTimeout, only when the ballad is set. The refuse log
 *     still says the tune keeps. ui.js is not this ticket.
 *  B. A set ballad that already got its playing: no job, no extra
 *     tally, no food, and no cooldown even after the old 2s timer
 *     would have fired. The log names the already-played night.
 *  C. Rest still works after the refuse.
 *  D. A first playing still fills the hat and stamps balladPlayed.
 *     Two tellings still refuse (no ballad set). Trade still
...
const balladAt = doBlock.indexOf("a.id==='ballad' && balladSet() && balladPlayed");

ok(/Play it once a session/.test(cfg) && /hat by the fire always fills/.test(cfg),
...
ok(/THE BRIDGE BALLAD/.test(finishBlock) && /hums along/.test(finishBlock),
  'the pay log still fills the hat from everybody humming along');
ok(doAt >= 0 && timeoutAt > 0, 'doAction still starts the job with setTimeout');
ok(balladAt >= 0 && balladAt < timeoutAt,
  'HV-281: doAction refuses a set already-played ballad before the timer');
...
(async () => {
  ...
  ok(!played.job && !played.btnOn && played.played && played.food === 10
    ...
    `HV-281: an already-played ballad does not start the job (job ${played.job}, food ${played.food}, plays ${played.tally})`);
  ...
  ok(first.food === 20 && first.played && first.tally === 1 && first.cd > Date.now(),
    `a first playing still fills the hat and still takes the lock (food ${first.food}, plays ${first.tally})`);
