 ...
 *
 * HV-234 already stamps the day. Flyers still pay the shop's +3; the
...
ok(/var unkind=j\.id==='flyers'&&gentrifyHostile\(\)/.test(body) && /if\(unkind&&k==='morale'\) amt=0/.test(body),
  'HV-186: the odd-job finisher reads gentrifyHostile on flyers and zeros morale');
...
(async () => {
  ...
  const runFlyers = (days, afterCard, weather = 'clear') => page.evaluate(({ days, afterCard, weather }) => {
    G.days = days;
    G.oddJobDay = -1;
    G.weather = weather;
    G.goodwill = 10; G.morale = 40; G.rep = 10;
    ...
    const before = { gw: G.goodwill, mo: G.morale, rep: G.rep };
    const j = todaysJob();
  ...
  ok(hostile.dRep === 3 && !/kind word/i.test(hostile.log) && !/\+0😊/.test(hostile.log),
    'hostile flyers keep honest-work Word only and omit +0 morale');

  const heat = await runFlyers(1, true, 'heat');
  ok(heat.dGw === 2 && heat.dMo === 0 && heat.dRep === 3 && !/kind word/i.test(heat.log),
    `hostile heat keeps the weather cut before removing kindness (${heat.dGw}/${heat.dMo})`);

  // D — a quiet day still gets the kind owner
  const quiet = await runFlyers(1, false);
  ok(quiet.dGw === 3 && quiet.dMo === 4 && quiet.dRep === 4 && /kind word/i.test(quiet.log),
    `a quiet day still pays +3 / +4 (${quiet.dGw}/${quiet.dMo})`);

  // E — other postings still pay after the card
  const depot = await t(() => {
    G.days = 0; G.oddJobDay = -1; G.gentrifyDay = 0; G.goodwill = 10; G.rep = 10;
    G.weather = 'clear'; G.structures.toolbox = false;
    finishAction(oddJobAction());
    return { job: todaysJob().id, gw: G.goodwill, rep: G.rep };
  });
  ok(depot.job === 'depot' && depot.gw === 15 && depot.rep === 14,
