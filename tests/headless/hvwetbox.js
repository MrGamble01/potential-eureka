...
ok(/snapActive\(\)/.test(pantry) && pantry.indexOf("weather==='rain'") < pantry.indexOf('snapActive()'),
  "HV-203's frozen box still stands, and the rain branch reads first");
...
(async () => {
  ...
  ok(snap.snap && snap.food === 10 && snap.fills === 0,
    `a named snap under a clear sky still freezes the box (HV-203), not this card's rain (food=${snap.food})`);
