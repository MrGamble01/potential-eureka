 ...
 *  E. Kind Stranger's rain is still HV-194's soaked bag.
 *  Z. Zero page errors.
 *
...
(async () => {
  ...
  await page.addInitScript(() => {
    if (!sessionStorage.getItem('hvwetgrant-init')) {
      sessionStorage.setItem('hvwetgrant-init', '1');
  ...
  const stranger = await t(() => {
    ...
    Math.random = real;
    const lines = Array.from(document.querySelectorAll('.log-line')).map(d => d.textContent);
    return { food: G.food, log: lines.join(' ') };
  });
  ok(stranger.food === 0 && /soaked it through/.test(stranger.log),
    `Kind Stranger's rain is still HV-194's soaked bag, not the crate's half (food=${stranger.food})`);
