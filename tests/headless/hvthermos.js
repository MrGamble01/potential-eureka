...
(async () => {
  ...
  const passed = await t(() => {
    ...
    finishAction({ id: 'thermos' });
    return { before, after, one, two: loadThermos().uses, used: linkDoneToday('thermos') };
  });
  ...
  await page.waitForTimeout(3000);
  const back = await t(() => ({ th: loadThermos(), used: linkDoneToday('thermos') }));
