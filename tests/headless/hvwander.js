 ...
 *
 * Homeless Village runs its own rAF loop. frame() gets a raw millisecond
 * dt (clamped to 100). spawnFigure stores `speed` as units per 60fps
 * frame, so the step has to be `speed * (dt/16.667)`. It used to read
 * `speed * dt * .016 * 60` (= speed * dt * 0.96). The `.016` is a frame
 * duration in seconds standing where a ms→frames divisor belongs, so
 * the step came out exactly 16× too fast.
 *
 * A resident whose `speed` reads 0.02 units/frame moved 0.32 — 19.2
 * units/second against the player's 4.2, crossing the whole 22-unit
 * camp in about a second. Worse, a 0.32 step overshoots the 0.1-unit
 * "arrived" test, so a resident that lands 0.1–0.22 short of its
 * target ping-pongs across it for the whole 3–8s until the next
 * target is picked.
 *
 ...
 * headless rAF's own clock — the whole claim is about what one known
 * dt does. No window.__ hook.
 */
...
const FRAME = 1000 / 60;   // ms for one 60fps frame — frame()'s dt unit
