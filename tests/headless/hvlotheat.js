 ...
 *     gardenh + weather==='heat' and applies 0.75. The depot,
 *     scrapyard and flyer heat cuts are their own tickets and
 *     stand untouched beside it.
 ...
 *     and the scrapyard pay their own tickets' scorcher
 *     rates, not the lot's.
...
ok(/j\.id==='depot' && G\.weather==='heat'/.test(body)
  && /j\.id==='scrapyd' && G\.weather==='heat'/.test(body)
  && /j\.id==='flyers' && G\.weather==='heat'/.test(body),
  'the depot / scrapyard / flyers heat cuts (#942 / #941 / #945) still stand beside it');
ok(!/j\.id==='gardenh' && G\.weather==='rain'/.test(body)
  && !/j\.id==='gardenh' && G\.weather==='cold'/.test(body),
  'rain and cold on the lot are not this ticket');
...
(async () => {
  ...
  ok(depot.id === 'depot' && depot.goodwill === 3,
    `the depot pays its HV-247 scorcher lift (+3), not the lot's cut (${depot.goodwill})`);

  const flyers = await shift('heat', 1);
  ok(flyers.id === 'flyers' && flyers.goodwill === 2 && flyers.morale === 53,
    `flyers pay their HV-250 scorcher walk (+2 / +3) (${flyers.goodwill} / ${flyers.morale})`);

  const yard = await shift('heat', 3);
  ok(yard.id === 'scrapyd' && yard.scraps === 3 && yard.cans === 1,
    `the scrapyard pays its HV-246 scorcher haul (+3 / +1) (${yard.scraps} / ${yard.cans})`);
