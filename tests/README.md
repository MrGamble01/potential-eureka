...
| Hub games | `undo2048`, `w5share`, `cycles3`, `snakepace` (SNAKE-1: Snake's Pace button names the pace the run is really ticking at — a daily run is locked to Classic, and the button used to keep advertising your saved Blitz and take a click it could not honour. Measures the scheduled loop delay from the test side rather than trusting the label) |
