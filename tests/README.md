...
| Age of War HUD | `aowbanner` (AOW-19b: the war-banner pill row keeps showing which banner is in force after a difficulty change from ⚙ Settings; the handler repaints only `button[data-diff]` pills. Non-vacuous: fails 4 checks by name on the old `.aow-diff button` fan-out) |
