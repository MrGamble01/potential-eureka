...
| Arcade power-ups | `sticky` (ARC-10: Neon Breaker's legend names every power-up it can actually drop, and the ⊙ sticky paddle — the one that changes the controls — says how to let the ball go. Catches a real one mid-run with a stubbed `Math.random`) |
