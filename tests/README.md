...
| Hub meta-layer | `daily` (7-game shared-seed challenge), `rivals` + `rivalsaow` + `rivalsflag` (share codes incl. all six flagship records), `ach`/`ach2` (achievements + completionist), `coins`, `insights`, `search`, `resume`, `theme`, `focus`, `shortcuts`, `patchnotes`, `backup` (whole-arcade backup/restore), `hofcard` (PNG score card), `escdialogs` (UI-7: Esc closes the two settings dialogs — Connect Your Calendars and Google Tasks — the way it already closed the cheat-sheet and the patch notes, and cancels rather than saves) |
