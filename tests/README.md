...
| Hub games | `undo2048`, `w5share`, `cycles3`, `astkeys` (AST-1: Vector Storm’s key map survives leaving the window — a turn or a thrust held when focus goes no longer flies the ship on its own; the un-gated keyup guarded alongside so it can’t regress into `whenViewActive`) |
