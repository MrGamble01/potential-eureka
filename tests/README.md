...
| Game economy | `economy` (models Grow Op's grow/trim/sell pipeline from the real constants and prints where income is capped; found three upgrades that add nothing at their own point of sale) |
...
**The battery preflights the server.** `run.sh` checks `$BASE` before it
starts and exits **2** (not 1) with the reason if it can't reach it — a
stopped server would otherwise fail all 128 suites with connection errors
after twenty-five minutes, reading exactly like 128 broken pages. Exit 2
marks it as an environment problem rather than a product failure, the same
distinction `audit.js` draws for sandbox network errors. `hubstore.js`
carries the same check for standalone runs.
