...
| Startup Tycoon | `tycdeal` (TYC-60: the deal-value multiplier is recomputed from prestige × Enterprise tier × live boosts instead of assigned by five owners. Found because every reload after the first IPO wiped the Enterprise Tier — a Season 2 company with Lv2 booted at $19 a ship instead of $154, with the HUD inventing a "Penalty ×0.13" to close the sum. Three layers, no hook: a source guard that allows exactly one assignment to `dealValueMult`; the production event pool, investor table and Enterprise entry replayed through `vm` with the overlaps that lost the tier (Demo Day + purchase, Corp Dev + Demo Day), the pre-fix arithmetic replayed beside them to show it ends at ×1; and real boots from seeded Season 2 saves reading the per-ship HUD, including buy → autosave → reload) |
| Startup Tycoon | `tycspeed` (TYC-61: the engineer-speed multiplier is recomputed from the five ideation upgrades × the Standup × live boosts, in the order the loader always applied them. Found because the Hackathon event ended by assigning `ideaSpeedMult = 1`, erasing every ideation upgrade on the floor until the next reload, and Faster Ideation assigned `1 + lv × 0.25` over anything bought before it. Same three layers as `tycdeal`: a one-assignment source guard; the production `UPGRADES` and `EVENT_POOL` tables replayed through `vm` with the Hackathon, a Pivot under an Outage burst, and the purchase-order case, the pre-fix arithmetic replayed beside them; and real boots reading the multiplier back through the save's `offlineRatePerSec`, which is written from it every 5 s) |
| Startup Tycoon | `tycrace` (TYC-62: `raceWonSeason` — whether you rang the bell before the rival — now rides the `rivalRace` schema entry. It lived only in memory, so any reload between the bell and the IPO rated a clean win SELL, reset the BUY streak that feeds the dividend, and banked one retro learning instead of two. Hook-free: seeds a save that has already won the race, presses the real IPO button, and reads the analyst chip and the saved rating back; the lost-race seed still rates SELL, and the pre-fix page rates the won seed SELL too) |
| Startup Tycoon | `tycipo` (TYC-63: the bell closes the books. `doIPO()` reset the company to $200 × season but left everything in flight where it was — an on-site engagement, a launch in polish and a hackathon paid old-season purses into the new company a minute later, an outage carried its page and its −6 morale burn-out across, a moonshot auto-lost against a reset race, the factoring desk kept diverting, the hedge rode into a race it never insured, every open pitch stayed up with its clock running, and Jeff Jackets churned $60+ every ten seconds out of a $200 company. `settleSeasonAtIPO()` settles the wagers and shelves the rest before the reset. Hook-free: seeds a Season 2 save with all of it in flight, presses the real IPO button, waits past every timer, and checks the new company holds exactly its $400 start with nothing left on the chips, in the feed, or in the save) |
| Startup Tycoon | `tycpb` (TYC-64: the season's personal best is played time. `triggerWin()` measured "Time to goal" from the page-load instant, never persisted and never reset at the IPO — a reload just before the bell recorded a 0:30 PB, and a second season in one tab carried the first's minutes. `seasonPlaySec` now accumulates frame dt, rides the save, and resets at the bell. Hook-free: seeds a season one deal short of its goal with an hour already played, lets a real BDR close the gap, and reads "Time to goal" off the real win card and the PB out of the save; the clock keeps counting across a reload; the IPO zeroes it) |
| Startup Tycoon | `tycwalk` (TYC-65: in walk mode, "click to fix" fixes. The bugged-desk label shows in both camera modes, but the top-down pointerdown handler — the only one that tested bugged desks — returned early in walk mode and the walk handler started a camera orbit instead. The walk handler now raycasts the click through the walk camera and lets a hotfix win over an orbit. Bugs are random and deliberately unsaved, so the suite holds the fix at the source layer (the walk handler tests bugged desks through `pcam` before taking the pointer) and drives real walk-mode clicks and drags on the page to prove they run clean) |
| Startup Tycoon | `tycseat` (TYC-66: every hire goes back to their own desk on a reload. Spawns took the first free desk in `plots` order and the save remembered only counts, so a reload reseated the roster in schema order — engineers, BDRs, AEs, then the Lead Engineer — and when a BDR or AE landed on the flex desk the Lead Engineer had held, `spawnVP()` found no seat and the promotion (up to $21,500) silently vanished. Seats now ride the save by position. Hook-free and click-driven: hire two engineers, promote the OG, buy the Sales Corner, hire two BDRs, reload — the Lead Engineer survives, everyone is at their own desk, and the hire panel still offers the next promotion; a legacy save without seat fields loads by fallback and starts remembering them) |
