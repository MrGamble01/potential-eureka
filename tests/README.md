...
| Age of War | `aowbanner` (AOW-19b: the war-banner pill row keeps showing which banner is in force when the difficulty changes; the Settings handler no longer fans out over `.aow-diff button`; the row stays styled like — but separate from — the difficulty pills) |
