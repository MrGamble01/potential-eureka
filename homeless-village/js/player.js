...
function finishAction(a){
  ...
  if(a.id==='scavenge'){
    ...
    if(!annivCounts()){ log('\ud83d\udd6f\ufe0f Nobody has counted the winters yet \u2014 three looks at the snapshot and the candle comes out.'); }
    else if(annivMarked){ log('\ud83d\udd6f\ufe0f The candle already burned today \u2014 the winters keep.'); }
    else {
      annivMarked=true;
      var ad=annivDish();
      var av=loadHvAnniv();
      ...
      G.food=(G.food||0)+ad;
      log('\ud83d\udd6f\ufe0f THE BRIDGE ANNIVERSARY \u2014 the winters counted, and a candle lit to prove it. Folks come by the flame with something for the pot: +'+ad+'\ud83c\udf5e');
      floatText('+'+ad+'\ud83c\udf5e');
      saveGame();
    }
