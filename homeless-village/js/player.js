...
function finishAction(a){
  ...
  if(a.id==='scavenge'){
    ...
    else if(linkDoneToday(a.id)){ log('\ud83e\uded6 The thermos made its round already \u2014 it refills tomorrow.'); }
    else {
      markLinkDone(a.id);
      var tp=thermosPower();
      var tt=loadThermos();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83d\ude97 Marisol already came by today \u2014 she has a garage to run.'); }
    else {
      markLinkDone(a.id);
      var md=marisolDish();
      var mm=loadMarisol();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83c\udf82 The reunion already went off today \u2014 people have places to be.'); }
    else {
      markLinkDone(a.id);
    ...
    else if(linkDoneToday(a.id)){ log('\ud83d\udcf7 The snapshot got its look today \u2014 it fades if you stare.'); }
    else {
      markLinkDone(a.id);
      var sd=snapshotDish();
      var ss=loadHvSnap();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83d\udd6f\ufe0f The candle already burned today \u2014 the year keeps.'); }
    else {
      markLinkDone(a.id);
      var ad=annivDish();
      var av=loadHvAnniv();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83d\udcd3 The notebook got its leaf-through today \u2014 the names keep.'); }
    else {
      markLinkDone(a.id);
      var gd=notebookDish();
      var gb=loadHvGb();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83e\ude91 The bench got its sit today \u2014 the seat keeps.'); }
    else {
      markLinkDone(a.id);
      var bd=hvBenchDish();
      var bb=loadHvBench();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83d\udd25 The story got its telling tonight \u2014 the fire remembers.'); }
    else {
      markLinkDone(a.id);
      var fd=hvStoryDish();
      var fs=loadHvStory();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83c\udfb8 The ballad got its playing tonight \u2014 the tune keeps.'); }
    else {
      markLinkDone(a.id);
      var bd2=balladDish();
      var bs2=loadHvSong();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83d\udce6 The can got its dig today \u2014 the piling keeps it.'); }
    else {
      markLinkDone(a.id);
      var cd2=canDish();
      var cn2=loadHvCan();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83c\udfa8 The panel got its stand today \u2014 the paint is still going on.'); }
    else {
      markLinkDone(a.id);
      var pd2=panelDish();
      var pn2=loadHvPanel();
    ...
    else if(linkDoneToday(a.id)){ log('\ud83e\udded Somebody already got the walk tonight \u2014 the next newcomer gets theirs tomorrow.'); }
    else {
      markLinkDone(a.id);
      var wd2=walkDish();
      var wk2=loadHvWalk();
    ...
    else if(linkDoneToday(a.id)){ log('\u270d\ufe0f A name went up tonight \u2014 the wall grows at the pace people stay.'); }
    else {
      markLinkDone(a.id);
      var md2=markDish();
      var mk2=loadHvMark();
    ...
    else if(linkDoneToday(a.id)){ log('\u26f1\ufe0f Somebody has already had their hour in the dry corner tonight \u2014 the roof keeps, and so does the habit.'); }
    else {
      markLinkDone(a.id);
      var dd2=dryDish();
      var dr2=loadHvDry();
