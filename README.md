...
The hub also has a few non-game utilities: a productivity view (notes, todos, pomodoro, bookmarks), a "Studio Crew" org chart, and a Hall of Fame view for high scores, achievements, and a local-only "Your arcade year" panel (launches/playtime per game — nothing leaves the browser).
...
| `eureka-stats` | `js/telemetry.js` — per-view launches + minutes for the 14 in-page arcade/toy views, shown as "Your arcade year" in the HOF |
