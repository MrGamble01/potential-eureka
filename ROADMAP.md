...
- **~~HVALE-6 · Hearthvale: the work roster~~** ✅ — HVALE-2 (mastery) and HVALE-3 (natures) both promised that *who* works *where* is a matching puzzle, but posts were only ever handed out first-come by `assignJobs()`, so the puzzle had no move: a Grandmaster Green thumb could spend the whole run at the quarry and the player could do nothing about it. A worker building's panel now offers **👤 Post a worker / Reassign the post**, opening a roster of every villager **ranked by what they would actually produce at that bench**, each row stating their nature, mastery rank and current post. Picking someone who already holds a post **trades posts** rather than taking one, so filling a bench never empties another. Persisted through the existing `jobId`, a `postings` counter rides the save, and 🧭 **The Right Hands** lands at five postings. In passing, the production chain was **three copies** — `runProduction()`, `cycleNet()` (the HUD's net-per-cycle) and the inspect panel each re-derived the same eight multipliers — so a roster quoting a fourth would have been drift waiting to happen: all of them now call one `projectedProd(b, k, worker)`, verified end to end (panel says 32, HUD net says 31, the town banks exactly 31 after a cycle). The hook-free `hvroster` suite (22/22) joins the committed battery.
