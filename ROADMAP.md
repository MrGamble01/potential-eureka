...
### Age of War
- **IDEA-AOW-1 · Endless/Survival mode** — gate the win check (`:1489`) behind a flag; score = waves survived. *(Partial: `aow-best-run` — fastest win time per difficulty — is now recorded and shown on the victory overlay; still open is the endless/survival mode itself.)*
...
| `aow-achievements`, `aow-difficulty`, `aow-muted`, `aow-welcome-seen`, `aow-best-run` | Age of War | ✓ (`aow-best-run` now written on victory — fastest time per difficulty; endless/survival mode itself is still open, IDEA-AOW-1) |
| `drug-lab-v1` | Grow Op | ✓ |
