...
- Still open: **IDEA-ARC-3 · Stacker** · **IDEA-ARC-6 · Word Cascade** (letters on the Tetris gravity/lock loop) · **IDEA-ARC-7 · Vector Defense** (tower defense in Asteroids' vector style; cap scope: 2 turrets, fixed path, ~8-10 waves).
...
- **IDEA-AOW-4 · Wall/Barricade unit** (`role:'wall'`, skip attack branch) · **~~IDEA-AOW-5 · Unit veterancy~~** ✅ shipped Aug 2026 (25s = Veteran +10% dmg, 60s = Elite +20%, gold chevrons above the HP bar, all four attack sites flow through `vetDmg()`) · **~~IDEA-AOW-6 · Turret targeting modes~~** ✅ shipped Aug 2026 (Near/Weak/Strong cycle per turret, mode survives upgrades) · **IDEA-AOW-7 · Overtime sudden-death** (escalating base chip damage after ~6 min).
