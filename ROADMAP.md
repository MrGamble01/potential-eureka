...
### Age of War
- **IDEA-AOW-1 · Endless/Survival mode** — gate the win check (`:1489`) behind a flag; score = waves survived. The `aow-best-run` half is done: the Reset button's key is now written on every game-over (ranked by kills, tiebroken by survival time) and shown on the game-over overlay; the endless/survival mode itself is still open.
...
| `aow-achievements`, `aow-difficulty`, `aow-muted`, `aow-welcome-seen`, `aow-best-run` | Age of War | ✓ (`aow-best-run` now written on every game-over) |
| `drug-lab-v1` | Grow Op | ✓ |
